...
export class AnthropicProvider implements ILLMProvider {
    ...
    async invoke(prompt: string, config: AnthropicConfig): Promise<LLMResponse> {
        const { model, temperature, maxTokens, topK, topP, 
                thinking, webSearch, providerOptions, stream } = config;
        ...
        if (thinking && response.usage) {
            ...
            thinkingTokens = Math.max(0, (response.usage.output_tokens || 0) - contentTokenEstimate);
        }
