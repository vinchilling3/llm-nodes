...
export class OpenAIProvider implements ILLMProvider {
    ...
    async invoke(prompt: string, config: OpenAIConfig): Promise<LLMResponse> {
        const { model, temperature, maxTokens, topP, frequencyPenalty, 
                presencePenalty, reasoning, webSearch, providerOptions } = config;
        ...
        if (useResponsesAPI) {
            ...
            try {
                ...
                if (Array.isArray(response.output)) {
                    // If output is an array, concatenate text items
    ...
    private async useChatCompletions(prompt: string, config: OpenAIConfig): Promise<LLMResponse> {
        const { model, temperature, maxTokens, topP, frequencyPenalty, 
                presencePenalty, providerOptions } = config;
